...
<p align="center">
  ...
  <a href="docs/NOTIFICATIONS.md">更新通知</a> ·
  <a href="docs/PRIVACY.md">隐私与数据</a> ·
  <a href="https://github.com/GpsLypy/CommitBrief/issues">反馈问题</a>
</p>

CommitBrief 面向需要写日报、周报和阶段汇报的开发者。添加电脑上的 Git 仓库，选择时间范围，就能查看按项目、模块和功能点整理的工作记录，核对原始提交，再编辑、复制或导出 Markdown / TXT 汇报。
...
<details>
<summary>查看深色模式、大模型设置、报告历史与推送设置</summary>

![CommitBrief 深色模式](.github/assets/workspace-dark.png)

![CommitBrief 大模型设置](.github/assets/ai-settings.png)

![CommitBrief 报告历史](.github/assets/report-history.png)

![CommitBrief 推送设置](.github/assets/notification-settings.png)
...
1. 打开 [最新版本](https://github.com/GpsLypy/CommitBrief/releases/latest)，下载 `CommitBrief-0.3.1-macos-arm64.dmg`。
...
目前没有 Intel 版、App Store 版或应用内自动安装更新功能。
...
| 阅读与 Markdown | 阅读模式按标题和列表简洁排版，Markdown 模式可编辑原文；复制跟随当前模式。 |
| 双格式导出 | `.md` 保留原文，`.txt` 移除 Markdown 标记并保留报告结构。 |
| 可选 AI 总结 | 使用自己的 API Key 和兼容接口，把提交整理为中文，归并相关工作。 |
| 草稿保护 | 范围变化时保留手动草稿并提示；重新整理或 AI 生成后可恢复上一份报告。 |
| 报告历史 | 查询最近 90 天的本地与 AI 报告版本，支持筛选、预览、恢复、删除和导出。 |
| 更新通知 | 可选 macOS 系统通知与 Webhook，只发送报告标题、范围和统计摘要。 |
| 版本检查 | 从侧栏手动检查 GitHub 最新正式版本，或直接打开项目主页。 |
...
5. **完成汇报**：切换阅读或 Markdown 模式，复制到工作文档，或导出 `.md` / `.txt` 文件。
...
支持 **OpenAI Responses 兼容接口**，服务地址和模型 ID 均可自定义，也可连接提供兼容接口的本机模型服务。

打开“模型设置”，填写服务商提供的地址和 API Key，可通过兼容的 `GET /models` 接口读取模型列表，也可手动填写模型 ID。选择“简洁汇报”或“详细汇报”，保存后手动点击“智能总结”。API Key 保存在 macOS 钥匙串；不接入模型也可以使用全部本地整理功能。
...
**是否保存历史周报？** 手动重新整理或智能总结成功后，应用会保存报告版本，可从标题栏历史按钮查询、恢复或导出。历史保留最近 90 天并存储在本机工作区文件中。

**大型仓库有哪些限制？** 单仓库单次最多读取 10,000 条提交；达到上限时会显示提示。请缩短统计范围以获得完整汇报。AI 服务还可能有独立的上下文与额度限制。

**还有哪些功能暂未提供？** 当前面向个人本地使用，未提供账户、云端同步、团队协作、定时生成、自动安装更新或 Intel 安装包。仓库目录移动后需要重新添加；从应用移除项目不会删除仓库。
